...
【利用条件】
個人・法人とも配信・動画制作・収益化、自分または自社用の改変、
自用のインストール・バックアップが可能です。クレジットは任意です。
ツールそのもの・改変版の第三者への配布、改変版の販売、自作発言は禁止します。
制作した配信・動画等の公開・収益化は可能です。
詳細はLICENSEを確認してください。リポジトリではルートLICENSE、
配布物では同梱LICENSEが規約本文です。
...
TwitCastingの番号付き匿名コメントにも対応しています。設定をONにすると、
匿名番号付きの表示名を配信単位で個別に記録し、同じ匿名番号の初回コメント
だけをBIG表示します。同じ番号の2回目以降は通常サイズです。匿名番号そのもの
は画面には表示しません。

※ Kickは参考対応です。Kick側の仕様変更や、わんコメ側の対応状況によって
コメントを取得できない場合があります。初コメBIG側のKick表示処理は実装して
いますが、コメント取得そのものはわんコメ側に依存します。
...
番号付き匿名コメントは匿名番号付きの表示名ごとに個別判定します。匿名番号の
数字部分だけを切り出したり、userId、screenNameのtw1/tw2/tw3を使ったりは
しません。
...
コメント表示は light / 標準フォント / 32px / 64px / 匿名BIG OFF / 枠OFFの既定値で
続きます。導入と有効化は公式プラグイン画面で行い、設定画面も同じ公式
プラグイン画面から
開きます。

設定項目は次の6つです。
- テーマ: light / dark / classic-gray / blue-gray / monochrome / amber / green-crt
  （ライト／ダーク／クラシックグレー／ブルーグレー／モノクロ／アンバー／グリーンCRT、既定light）
- レトロウインドウ枠: ON / OFF（既定OFF、固定タイトル「コメント」）
- フォント: 標準（游ゴシック）／メイリオ／太ゴシック（BIZ UDPゴシック）／
  丸ゴシック（M PLUS Rounded 1c）／明朝体
...
Fonts版丸ゴシック、TwitCasting番号付き匿名コメントの個別判定と匿名BIG
ON/OFF、gift色、owner表示、設定のリアルタイム
